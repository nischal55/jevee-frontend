...
const AllCategories = () => {
  ...
  useEffect(()=>{
    axios.get("https://dummyjson.com/products").then((res)=>{
     setCategorys(res.data.products)
 ...
 function deleteCategory(id){
  ...
  .then((res) => {
    toast(`Category deleted successfully`, { autoClose: 2000 });
...
function handleEditcategory(category) {
  navigate('', { state: { categorydata: category } });
}
...
const showResult = categorys.slice(startIndex, startIndex + limit);
 ...
 <thead className='border-b-[1.5px]'>
   ...
   <th className='w-1/6 p-2 text-center'>Name</th>
   <th className='w-1/6 p-2 text-center'>Image</th>
   <th className='w-1/6 p-2 text-center'>Icon</th>
   <th className='w-1/6 p-2 text-center'>Status</th>
   <th className='w-1/6 p-2 text-center'>Action</th>
   </tr>
 </thead>
 <tbody className='text-zinc-500'>
   {showResult.length > 0 ? showResult.map((category,index) => (
     <tr key={index} className='text-center'>
       <td className='w-1/6 p-2'>{category.id}</td>
       <td className='w-1/6 p-2'>{category?.title}</td>
       <td className='w-1/6 p-2'>
       <div className='flex justify-center items-center h-[80px] w-[80px] bg-cover mx-auto'>
       <img src={category?.thumbnail} alt="" className='object-contain h-full w-full'/>
       </div>
       </td>
       <td className='w-1/6 p-2'><div className='flex justify-center items-center h-8 w-8 bg-cover mx-auto'>
       <img src={category?.icon} alt="" className='object-contain h-full w-full'/>
       </div></td>
       <td className='w-1/6 p-2'>
  ...
  <span className="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">{categorys.status ?"Active":"Inactive"}</span>
</label>
       ...
       <button onClick={()=>deleteCategory(category.id)}><MdDeleteOutline className='bg-red-500'/></button>
       </div>
