...
const AllSubCategories = () => {
  ...
  useEffect(()=>{
    axios.get("https://dummyjson.com/products").then((res)=>{
      setSubCategorys(res.data.products)
 ...
 function deleteSubCategory(id){
  ...
  .then((res) => {
    toast(`Sub Category deleted successfully`, { autoClose: 2000 });
    setSubCategorys(subCategorys.filter((subCategory) => subCategory.id !== id));
  })
  .catch((err) => {
...
function handleSearch(e) {
  ...
  const filtered = subCategorys.filter((subCategory) =>
    subCategory.title.toLowerCase().includes(searchValue)
  );
  setSubCategorys(filtered);
}

function handleEditSubCategory(subCategory) {
  navigate('', { state: { subCategorydata: subCategory } });
}
...
const showResult = subCategorys.slice(startIndex, startIndex + limit);

    return (

        <>
          <div className="cardHead -z-[0] flex flex-col md:flex-row justify-between gap-4">
                <h2 className='text-3xl font-semibold text-[#34395E]'>Product Sub Category</h2>
                <div className='flex text-[12px] gap-2'>
                  ...
                  /
                  <h2 className='text-slate-500'>Product Sub Category</h2>
 ...
 <thead className='border-b-[1.5px]'>
   ...
   <th className='w-1/6 p-2 text-center'>Sub Category</th>
   <th className='w-1/6 p-2 text-center'>Slug</th>
   <th className='w-1/6 p-2 text-center'>Category</th>
   <th className='w-1/6 p-2 text-center'>Status</th>
   <th className='w-1/6 p-2 text-center'>Action</th>
   </tr>
 </thead>
 <tbody className='text-zinc-500'>
   {showResult.length > 0 ? showResult.map((subCategory,index) => (
     <tr key={index} className='text-center'>
       <td className='w-1/6 p-2'>{subCategory.id}</td>
       <td className='w-1/6 p-2'>{subCategory?.subCategory}</td>
       <td className='w-1/6 p-2'>{subCategory?.slug}</td>
       <td className='w-1/6 p-2'>{subCategory?.title}</td>
  ...
  <span className="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">{subCategory.status ?"Active":"Inactive"}</span>
</label>
       ...
       <td className='w-full py-8 flex justify-center items-center gap-1'>
        ...
        <button onClick={()=>handleEditSubCategory(subCategory)}><FaEdit /></button>
        </div>
       <div className='h-8 w-8 bg-red-500 flex justify-center items-center text-white rounded'>
       <button onClick={()=>deleteSubCategory(subCategory.id)}><MdDeleteOutline className='bg-red-500'/></button>
       </div>
